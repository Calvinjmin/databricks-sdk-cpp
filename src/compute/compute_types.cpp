...
ClusterStateEnum parse_cluster_state(const std::string& state_str) {
    ...
    if (upper_state == "PENDING")
        return ClusterStateEnum::PENDING;
    if (upper_state == "RUNNING")
        return ClusterStateEnum::RUNNING;
    if (upper_state == "RESTARTING")
        return ClusterStateEnum::RESTARTING;
    if (upper_state == "RESIZING")
        return ClusterStateEnum::RESIZING;
    if (upper_state == "TERMINATING")
        return ClusterStateEnum::TERMINATING;
    if (upper_state == "TERMINATED")
        return ClusterStateEnum::TERMINATED;
    if (upper_state == "ERROR")
        return ClusterStateEnum::ERROR;
