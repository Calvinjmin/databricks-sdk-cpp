...
AuthConfig AuthConfig::from_env() {
    ...
    if (!host_env)
        host_env = std::getenv("DATABRICKS_SERVER_HOSTNAME");
    if (!host_env) {
    ...
    if (!token_env)
        token_env = std::getenv("DATABRICKS_ACCESS_TOKEN");
    if (!token_env) {
...
AuthConfig AuthConfig::from_environment(const std::string& profile) {
    ...
    std::string combined_error = "Failed to load Databricks authentication configuration. "
                                 "Ensure either:\n"
                                 "  1. ~/.databrickscfg has a [" +
                                 profile +
                                 "] section with host and token, OR\n"
...
bool PoolingConfig::is_valid() const {
    return min_connections > 0 && max_connections >= min_connections && connection_timeout_ms > 0;
}

// ========== RetryConfig Implementation ==========

bool RetryConfig::is_valid() const {
    return max_attempts > 0 && initial_backoff_ms > 0 && backoff_multiplier > 0.0 &&
